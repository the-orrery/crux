...
BUILD_DIR="${CRUX_BUILD_DIR:-${ROOT}/build/pyinstaller}"
...
uv run --group freeze pyinstaller --noconfirm --onefile --clean \
  --paths "${ROOT}/src" --collect-submodules crux --collect-submodules gnomon \
  --collect-submodules orrery_heartbeat --name crux --distpath "${BUILD_DIR}/dist" \
  --workpath "${BUILD_DIR}/work/crux" --specpath "${BUILD_DIR}/spec" \
  "${ROOT}/scripts/crux_entry.py"
install -m 0755 "${BUILD_DIR}/dist/crux" "${OUTPUT_DIR}/crux-${platform}-${arch}"
if [[ "${SKIP_SMOKE:-0}" != "1" ]]; then
  smoke_root="$(mktemp -d)"
  CI=1 ORRERY_NO_UPDATE_CHECK=1 XDG_DATA_HOME="${smoke_root}/data" XDG_CACHE_HOME="${smoke_root}/cache" \
    "${OUTPUT_DIR}/crux-${platform}-${arch}" --help >/dev/null
  rm -rf "${smoke_root}"
fi
